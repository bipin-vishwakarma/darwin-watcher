...
$Java = Join-Path $Root 'app\src\main\java\com\darwin\sandboxdemo\MainActivity.java'
$Package = 'com.darwin.sandboxdemo'
$ApkUnsigned = Join-Path $Build 'app-unsigned.apk'
$ApkAligned = Join-Path $Build 'app-aligned.apk'
$ApkDebug = Join-Path $Build 'darwinbox-sandbox-debug.apk'
...
$rJava = Join-Path $Build 'gen\com\darwin\sandboxdemo\R.java'
Invoke-Native javac --release 8 -classpath $PlatformJar -d "$Build\classes" $rJava $Java
$classFiles = Get-ChildItem "$Build\classes" -Filter '*.class' -Recurse | ForEach-Object { $_.FullName }
Invoke-Native java -cp "$BuildTools\lib\d8.jar" com.android.tools.r8.D8 --min-api 23 --output "$Build\dex" @classFiles
